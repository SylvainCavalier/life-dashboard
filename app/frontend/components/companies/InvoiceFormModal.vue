  ...
  <BaseModal :title="`Nouvelle facture — ${company?.trade_name}`" @close="$emit('close')">
    <form @submit.prevent="save" class="space-y-6">
      ...
      <section v-if="clients.length">
        ...
        <select v-model="selectedClientId" @change="applyClient" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
      ...
      <section>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div class="flex items-center gap-2 self-end pb-2">
            <input v-model="form.tva_exempt" type="checkbox" id="invoice-tva-exempt" @change="onTvaExemptChange" class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
      ...
      <section>
        <div class="flex items-center justify-between mb-3 border-b pb-2">
          ...
          <button type="button" @click="addItem" class="text-blue-600 hover:text-blue-800 text-xs font-medium">+ Ajouter une ligne</button>
        </div>
      ...
      <div class="flex gap-2 justify-end">
        ...
        <button type="submit" :disabled="saving" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium disabled:opacity-50">
          {{ saving ? 'Generation...' : 'Generer la facture' }}
        </button>
      </div>
    </form>
  </BaseModal>
...
const save = async () => {
  ...
  try {
    const payload = {
      ...
      invoice_items_attributes: form.items.map((item, i) => ({
    ...
    await apiClient.post(`/companies/${props.company.id}/invoices`, payload)
    emit('saved')
  } catch (e) {
